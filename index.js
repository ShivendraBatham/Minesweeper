...
const getBombsFromNeighbour = (target, val) => {
    ...
    else if (val > 0 && val < 9 || val > 90 && val < 99 || val===10 || val===20 || val===30 || val===40 || val===50 || val===60 || val===70 || val===80 || val===19 || val===29 || val===39 || val===49 || val===59 || val===69 || val===79 || val===89) {
...
//Setting dataAttribute to the boxes on the basis of number of bombs in surroundings
...
//Checking whether the flagged boxes contain bomb or not
...
grid.addEventListener('contextmenu', e => {
    ...
    if(!gameOver) {
        ...
        //check if element is flagged, if flagged then remove flag and decrement the flag counter
        //otherwise add flag and increment the flag counter
        if(target.classList.contains('flag')) {
            target.classList.remove('flag');
            flagsCounter--;
            flagsLeft.innerHTML = totalFlags - flagsCounter;
        ...
        //if they contain bomb display 'you have won' message otherwise 'lose' message
